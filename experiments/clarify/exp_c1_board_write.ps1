# C-1: Board Write with Clarify Context
# Compare with B-4 (same minimal prompt, no clarify) to measure Clarify effect
...
Write-Host 'Starting C-1: Board Write with Clarify...'
...
try {
    ...
    $response | ConvertTo-Json -Depth 10 | Out-File -FilePath 'c:\Project\demo-app\experiments\clarify\exp_c1_result.json' -Encoding UTF8
    Write-Host 'Done. Saved.'
} catch {
    $sw.Stop()
    Write-Host ('ERROR after ' + $sw.Elapsed.TotalSeconds + 's: ' + $_.Exception.Message)
}
