# J-2 retry: Board Write + Clarify (Judge v2) - 600s timeout
...
Write-Host "Starting J-2 retry: Board Write + Clarify (Judge v2)..."
...
try {
    ...
    Write-Host "clarity=$($response.testCases.clarity) feasibility=$($response.testCases.feasibility) completeness=$($response.testCases.completeness) practicality=$($response.testCases.practicality)"
} catch {
    $sw.Stop()
